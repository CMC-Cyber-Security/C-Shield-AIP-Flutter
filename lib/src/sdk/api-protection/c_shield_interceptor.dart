...
import '../../internal/ap_normalizer.dart';
import 'c_shield_ap.dart';

/// An [http.BaseClient] that automatically signs every outgoing request
/// and verifies every incoming response using CShield AP (API Protection).
///
...
class CShieldInterceptor extends http.BaseClient {
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    ...
    final apHeaders = await CShieldAP.signRequest(method: request.method, path: path, body: body, contentType: contentType);
    request.headers.addAll(apHeaders);
    ...
    await CShieldAP.verifyResponse(statusCode: streamed.statusCode, path: path, headers: streamed.headers, body: responseBytes);
  ...
  static (Uint8List, String) _bodyAndContentType(http.BaseRequest request) {
    if (request is http.MultipartRequest) {
      ...
      return (APNormalizer.normalizeFields(request.fields), 'application/json');
    }
