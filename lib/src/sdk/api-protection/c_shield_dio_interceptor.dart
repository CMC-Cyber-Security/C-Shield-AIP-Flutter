...
class CShieldDioInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    try {
      ...
      options.headers.addAll(apHeaders);
  ...
  void onResponse(Response response, ResponseInterceptorHandler handler) async {
    ...
    try {
      ...
      await CShieldAP.verifyResponse(statusCode: response.statusCode ?? 0, path: response.requestOptions.uri.path, headers: _flattenHeaders(response.headers), body: rawBytes);
  ...
  static Uint8List _normalizeFormData(FormData form) {
    final fields = <String, String>{for (final e in form.fields) e.key: e.value};
    return APNormalizer.normalizeFields(fields);
  }
