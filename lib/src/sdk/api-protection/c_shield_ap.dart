...
import 'package:c_shield_aip/src/internal/ap_normalizer.dart';
import 'package:c_shield_aip/src/internal/platform/c_shield_aip_platform_interface.dart';

/// Low-level AP (API Protection) API.
///
...
class CShieldAP {
  ...
  /// final norm = await CShieldAP.normalizeBody(body: bytes, contentType: 'application/json');
  /// final payload = 'POST./api/v1/login.$ts.${norm['hash']}';
  /// final sig = await CShieldAP.sign(payload);
  /// // Attach to request: {'cs-timestamp': '$ts', 'cs-signature': sig}
  /// ```
  static Future<String> sign(String payload) => CShieldAipPlatform.instance.apSign(payload: payload);

  /// Verifies [signature] against [payload].
  ///
  /// Throws [CShieldException] with [CShieldErrorCode.apInvalidSignature] or
  /// [CShieldErrorCode.apTimestampExpired] on failure.
  static Future<void> verify({required String payload, required String signature}) => CShieldAipPlatform.instance.apVerify(payload: payload, signature: signature);
  ...
  ///
  /// Computed entirely in Dart (see [APNormalizer]) — byte-identical to the
  ...
  static Future<Map<String, dynamic>> normalizeBody({required Uint8List body, String contentType = 'application/json'}) async =>
      APNormalizer.normalizeBodyForSigning(body: body, contentType: contentType).toMap();
  ...
  static Future<Map<String, String>> signRequest({required String method, required String path, required Uint8List body, String contentType = 'application/json'}) async {
    final normalized = APNormalizer.normalizeBodyForSigning(body: body, contentType: contentType);
  ...
  /// Verifies the AP signature on an incoming response.
  ///
  ...
  static Future<void> verifyResponse({required int statusCode, required String path, required Map<String, String> headers, required Uint8List body}) async {
    ...
    if (timestampStr == null) {
      throw const CShieldException(CShieldErrorCode.apMissingHeader, "Not found cs-timestamp in response's header");
    }
    final timestamp = int.tryParse(timestampStr);
    if (timestamp == null) {
      throw const CShieldException(CShieldErrorCode.apTimestampExpired, 'Invalid cs-timestamp format');
    }
    if ((_nowSeconds() - timestamp).abs() > _responseFreshnessSeconds) {
      throw const CShieldException(CShieldErrorCode.apTimestampExpired, 'Timeout response handle');
    }

    final signature = lower['cs-signature'];
    if (signature == null) {
      throw const CShieldException(CShieldErrorCode.apMissingHeader, "Not found cs-signature in response's header");
    }

    // Hash the raw response bytes as-is, exactly like native validateResponse.
    final bodyHash = APNormalizer.sha256Hex(body);
