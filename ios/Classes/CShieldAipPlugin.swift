...
/// Mirrors CShieldAipPlugin.kt (android) — keep both in sync.
public class CShieldAipPlugin: NSObject, FlutterPlugin {

    private let sslBridge = SslBridge()
    private let apBridge = ApBridge()

    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "c_shield_aip", binaryMessenger: registrar.messenger())
        let instance = CShieldAipPlugin()
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case let m where m.hasPrefix("ap."):
            apBridge.handle(call: call, result: result)
        default:
